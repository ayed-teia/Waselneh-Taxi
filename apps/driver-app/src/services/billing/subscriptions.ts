...
export function subscribeToDriverInvoices(driverId: string, callback: (items: DriverSubscriptionInvoice[]) => void): Unsubscribe {
  return firebaseDB.collection('subscriptionInvoices').where('targetId', '==', driverId).onSnapshot((snapshot) => {
    const items = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() } as DriverSubscriptionInvoice));
