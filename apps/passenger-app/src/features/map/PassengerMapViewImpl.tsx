...
// Applies the Mapbox token on first map use. It used to run at router-load time,
  ...
  const promoteStyleStage = useCallback(
    (reason: 'timeout' | 'error') => {
      if (styleStage === 'primary') {
        const moveDirectToLocal = reason === 'error';
        console.warn(
          `${MAP_LOG_PREFIX} Primary style ${reason}; switching to ${moveDirectToLocal ? 'simplified' : 'alternate'} style.`
        );
        setStyleStage(moveDirectToLocal ? 'local-fallback' : 'alternate');
        setStyleLoaded(false);
        setError(null);
        setNotice(
          moveDirectToLocal
            ? isRTL
              ? 'تعذر تحميل خرائط Mapbox، يتم التحويل إلى نمط OSM المبسط.'
              : 'Mapbox is unavailable, switching to simplified OSM mode.'
            : isRTL
