...
const CreateSchema = z
  ...
  .refine(
    (value) => value.commissionRecordIds.length + value.subscriptionInvoiceIds.length > 0,
...
export const managerCreateOfficeStatement = onCall({ region: REGION }, async (request) => {
  try {
    ...
    if (!parsed.success)
      throw new ValidationError('Invalid office statement', parsed.error.flatten());
    const data = parsed.data;
    await assertManagerPermission(managerId, 'manage_pricing', { officeId: data.officeId });
    ...
    const recordRefs = [...new Set(data.commissionRecordIds)].map((id) =>
      db.collection('commissionRecords').doc(id)
    );
    const invoiceRefs = [...new Set(data.subscriptionInvoiceIds)].map((id) =>
      db.collection('subscriptionInvoices').doc(id)
    );
    return db.runTransaction(async (transaction) => {
      ...
      const commissionInputs = records.map((doc) => {
        ...
        const recordPeriod =
          value.createdAt instanceof Timestamp
            ? value.createdAt.toDate().toISOString().slice(0, 7)
            : '';
        if (
          !doc.exists ||
          value.officeId !== data.officeId ||
          value.status !== 'pending' ||
          recordPeriod !== data.periodKey
        )
          throw new ForbiddenError(
            `Commission ${doc.id} is not pending for this office and period`
          );
        return {
      ...
      const invoiceAmounts = invoices.map((doc) => {
        ...
        if (
          !doc.exists ||
          value.targetType !== 'office' ||
          value.targetId !== data.officeId ||
          value.periodKey !== data.periodKey ||
          !['pending', 'past_due', 'suspended'].includes(String(value.status ?? ''))
        )
          throw new ForbiddenError(`Invoice ${doc.id} is not payable for this office and period`);
      ...
      transaction.set(statementRef, {
        ...
        commissionRecordIds: recordRefs.map((ref) => ref.id),
        subscriptionInvoiceIds: invoiceRefs.map((ref) => ref.id),
        ...amounts,
        currency: 'ILS',
        status: 'pending',
        createdBy: managerId,
...
export const managerMarkOfficeStatementPaid = onCall({ region: REGION }, async (request) => {
  try {
    ...
    if (!parsed.success)
      throw new ValidationError('Invalid office statement payment', parsed.error.flatten());
    const db = getFirestore();
    ...
    if (!statementSnapshot.exists)
      throw new NotFoundError('Office statement', parsed.data.statementId);
    const scopedOfficeId =
      typeof statementSnapshot.data()?.officeId === 'string'
        ? (statementSnapshot.data()?.officeId as string)
        : '';
    await assertManagerPermission(managerId, 'manage_pricing', { officeId: scopedOfficeId });
    await db.runTransaction(async (transaction) => {
      ...
      if (data.status !== 'pending')
        throw new ForbiddenError('Only pending statements can be paid');
      const recordIds = Array.isArray(data.commissionRecordIds)
        ? data.commissionRecordIds.filter((id): id is string => typeof id === 'string')
        : [];
      const invoiceIds = Array.isArray(data.subscriptionInvoiceIds)
      ...
      const [subscription, officeInvoices] = await Promise.all([
        ...
        transaction.get(
          db.collection('subscriptionInvoices').where('subscriptionId', '==', subscriptionRef.id)
        ),
      ]);
      const included = new Set(invoiceIds);
      const otherStatuses = officeInvoices.docs
        .filter((doc) => !included.has(doc.id))
        .map((doc) => String(doc.data().status ?? ''));
      const reactivate =
        subscription.exists &&
        shouldReactivateSubscription(String(subscription.data()?.status ?? ''), otherStatuses);
      ...
      recordIds.forEach((id) =>
        transaction.update(db.collection('commissionRecords').doc(id), {
          status: 'settled',
      ...
      invoiceIds.forEach((id) =>
        transaction.update(db.collection('subscriptionInvoices').doc(id), {
          status: 'paid',
      ...
      if (reactivate) {
        ...
        transaction.set(
          db.collection('offices').doc(officeId),
          { subscriptionStatus: 'active', subscriptionUpdatedAt: FieldValue.serverTimestamp() },
          { merge: true }
        );
      }
    });
