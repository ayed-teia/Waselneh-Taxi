...
const PREFIX = 'subinv';
export const subscriptionInvoicePaymentSubject = (invoiceId: string) =>
  `${PREFIX}${createHash('sha256').update(invoiceId).digest('hex').slice(0, 40)}`;
export const isSubscriptionInvoicePaymentSubject = (value: string) =>
  value.startsWith(PREFIX) && value.length === 46;

export async function advanceSubscriptionInvoicePayment(
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (!subject.exists || subject.data()?.kind !== 'subscription_invoice')
      throw new ValidationError('Unknown payment subject');
    ...
    if (event.amountMinorUnits !== Math.round(Number(data.amountIls ?? 0) * 100))
    ...
    const [subscription, invoices] = await Promise.all([
      ...
      transaction.get(
        db.collection('subscriptionInvoices').where('subscriptionId', '==', subscriptionId)
      ),
    ]);
    ...
    const reactivated = shouldReactivateSubscription(
    ...
    transaction.update(invoiceRef, {
      status: 'paid',
      paymentMethod: 'card',
      paymentReference: event.providerChargeId,
      provider,
    ...
    transaction.set(
      subjectRef,
      { status: 'paid', eventId: event.eventId, updatedAt: FieldValue.serverTimestamp() },
      { merge: true }
    );
    ...
    if (data.targetType === 'driver' && typeof data.targetId === 'string') {
      transaction.set(
        db
          .collection('userNotifications')
          .doc(data.targetId)
          .collection('items')
          .doc(`subscription_receipt_${invoiceId}`),
        {
          ...
          invoiceId,
          read: false,
          titleAr: 'تم دفع فاتورة الاشتراك',
          titleEn: 'Subscription invoice paid',
          bodyAr: `تم استلام ${Number(data.amountIls ?? 0).toFixed(2)} شيكل. مرجع الدفع: ${event.providerChargeId}`,
          bodyEn: `NIS ${Number(data.amountIls ?? 0).toFixed(2)} received. Payment reference: ${event.providerChargeId}`,
          createdAt: FieldValue.serverTimestamp(),
        }
      );
    }
    if (reactivated) {
      ...
      if ((targetType !== 'driver' && targetType !== 'office') || typeof targetId !== 'string')
      ...
      transaction.set(
        db.collection(targetType === 'driver' ? 'drivers' : 'offices').doc(targetId),
